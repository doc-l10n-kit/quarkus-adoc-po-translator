 ...
 * @param group 
 * @param engine An engine ID, or the string `stock` to use the stock engine for the language combination (if available).
...
data class CreateProject (
    ...
    /* An engine ID, or the string `stock` to use the stock engine for the language combination (if available). */
    @Json(name = "engine")
    val engine: kotlin.Int,
    /* CAT tool server connection ID as stored in Globalese. */
    @Json(name = "cat-server-id")
    val catServerId: kotlin.Int? = null,
    /* CAT tool project ID as stored on the CAT tool server. */
    @Json(name = "cat-project-id")
